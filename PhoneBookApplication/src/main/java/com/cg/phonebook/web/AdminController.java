...
public class AdminController {
	...
	public Iterable<UserEntity> findAll(){
		Iterable<UserEntity> userList = adminService.findAllContacts();
		if(userList!=null) {
			return adminService.findAllContacts();
		}
		else {
	...
	public ResponseEntity<String> findDataByMailId(@PathVariable("email") String email,@PathVariable("password") String password) throws ResourceNotFoundException {
		boolean flag=false;
	...
	public AdminEntity addAdmin(@RequestBody AdminEntity data) throws ResourceNotFoundException {
		...
		if(user&&password1) {
			if(v==null) {
					if(data.getPassword().equals(data.getConfirmPassword())) {
						return adminService.saveAdmin(data);
					}
					else {
